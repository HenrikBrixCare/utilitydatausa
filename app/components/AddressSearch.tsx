...
type Profile = {
  ...
  weather: null | { status: string; forecastPeriods: Array<{ name: string; temperature: number | null; temperatureUnit: string | null; shortForecast: string | null; windSpeed: string | null; windDirection: string | null }>; alerts: Array<{ event: string; severity: string | null; headline: string | null }>; sourceUrl: string; limitation: string };
  limitation: string;
};
          ...
          {result.address && (
            <>
              ...
              <div className="live-result-grid expanded-result-grid">
                ...
                <article className="live-result-card"><div className="result-card-top"><span className="result-icon weather">☁</span><strong>NWS Weather & Alerts</strong><em className={result.weather?.status === "ok" ? "status-live" : "status-muted"}>{statusLabel(result.weather?.status ?? "error")}</em></div><h3>{result.weather?.alerts.length ? `${result.weather.alerts.length} active weather alert${result.weather.alerts.length === 1 ? "" : "s"}` : result.weather?.forecastPeriods[0]?.shortForecast ?? "Current forecast context"}</h3>{result.weather?.alerts.slice(0, 2).map((alert, index) => <p className="compact-result" key={`${alert.event}-${index}`}><strong>{alert.event}</strong>{alert.headline ? ` · ${alert.headline}` : ""}</p>)}{!result.weather?.alerts.length && result.weather?.forecastPeriods.slice(0, 2).map((period) => <p className="compact-result" key={period.name}>{period.name}: {period.temperature !== null ? `${period.temperature}°${period.temperatureUnit ?? ""} · ` : ""}{period.shortForecast ?? "Forecast"}{period.windSpeed ? ` · wind ${period.windSpeed}` : ""}</p>)}<div className="context-links"><a href="https://api.weather.gov" target="_blank" rel="noreferrer">NWS source</a></div><small>{result.weather?.limitation}</small></article>
