...
const resources = [
  { id: "census", status: "Live", cls: "status-live", title: "U.S. Census Bureau", text: "Address matching, coordinates and Census geography. UtilityDataUSA uses Census geocoding as the common starting point for a searched address.", href: "https://geocoding.geo.census.gov/geocoder/", links: [["Open Census Geocoder", "https://geocoding.geo.census.gov/geocoder/"],["API documentation", "https://geocoding.geo.census.gov/geocoder/Geocoding_Services_API.html"]] },
  { id: "epa", status: "Live", cls: "status-live", title: "U.S. EPA", text: "Environmental source context and nearby Facility Registry Service records. Useful for screening, not a complete environmental due-diligence determination.", href: "https://www.epa.gov/enviro", links: [["EPA Envirofacts", "https://www.epa.gov/enviro"],["FRS API", "https://www.epa.gov/frs/frs-api"]] },
  { id: "fema", status: "Live", cls: "status-live", title: "FEMA", text: "Official flood-hazard context from the National Flood Hazard Layer. A point lookup is not a survey, elevation certificate or insurance determination.", href: "https://www.fema.gov/flood-maps", links: [["FEMA Flood Maps", "https://www.fema.gov/flood-maps"],["NFHL service", "https://hazards.fema.gov/arcgis/rest/services/public/NFHL/MapServer"]] },
  { id: "usgs", status: "Live", cls: "status-live", title: "U.S. Geological Survey", text: "Hydrologic monitoring context and nearby monitoring locations. USGS monitoring sites do not identify a property's water or sewer provider.", href: "https://waterdata.usgs.gov/", links: [["USGS Water Data", "https://waterdata.usgs.gov/"],["Water Data APIs", "https://api.waterdata.usgs.gov/docs/"]] },
  { id: "phmsa", status: "Public context", cls: "status-context", title: "PHMSA / NPMS", text: "UtilityDataUSA now carries county/ZIP-aware NPMS public context and direct official follow-up. NPMS is not exact line locating and does not include gas distribution or gathering lines.", href: "https://pvnpms.phmsa.dot.gov/", links: [["NPMS Public Viewer", "https://pvnpms.phmsa.dot.gov/"],["Operator directory", "https://www.npms.phmsa.dot.gov/FindWhosOperating.aspx"],["PHMSA public portal", "https://www.npms.phmsa.dot.gov/GeneralPublic"]] },
  { id: "811", status: "Follow-up", cls: "status-followup", title: "811 / State One-Call", text: "The required safety handoff before digging, drilling or excavating. UtilityDataUSA must never be used instead of the applicable state one-call process.", href: "https://call811.com/811-in-your-state/", links: [["811 in your state", "https://call811.com/811-in-your-state/"],["Call811.com", "https://call811.com/"]] },
  { id: "eia", status: "Public context", cls: "status-context", title: "U.S. Energy Information Administration", text: "UtilityDataUSA now connects Census county/state geography with EIA context. EIA-861 service-territory data can indicate utility presence by county/state but does not prove service at a specific address.", href: "https://www.eia.gov/electricity/data/eia861/", links: [["EIA Open Data", "https://www.eia.gov/opendata/"],["EIA-861 Service Territory", "https://www.eia.gov/electricity/data/eia861/"],["Free API key", "https://www.eia.gov/opendata/register.php"]] },
  { id: "state", status: "Expanding", cls: "status-expanding", title: "State & Local Sources", text: "Public utility commissions, state GIS portals, counties, assessors, cities, municipal utilities, water/sewer districts and local permit systems vary by location.", href: "https://www.usa.gov/state-governments", links: [["Find your state government", "https://www.usa.gov/state-governments"]] }
];
  ...
  return (
    <main className="product-page resources-page">
      ...
      <section className="product-section white" id="data-sources"><div className="portal-container">
        ...
        <div className="resource-grid">{resources.map((resource) => <article className="resource-card clickable-resource-card" id={resource.id} key={resource.title}>
          <a className="resource-card-main-link" href={resource.href} target="_blank" rel="noreferrer" aria-label={`Open ${resource.title} official resource`}></a>
          <span className={`status-pill ${resource.cls}`}>{resource.status}</span>
          <h3>{resource.title}</h3>
          <p>{resource.text}</p>
          <div className="resource-links">{resource.links.map(([label,url]) => <a href={url} target="_blank" rel="noreferrer" key={url}>{label}</a>)}</div>
        </article>)}</div>
      </div></section>
