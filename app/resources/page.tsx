...
const resources = [
  ...
  { id: "nws", status: "Live", cls: "status-live", title: "National Weather Service", text: "Weather forecasts and active alerts for the location, checked separately.", links: [["NWS API", "https://www.weather.gov/documentation/services-web-API"],["Active alerts", "https://api.weather.gov/alerts/active"]] },
  { id: "terrain", status: "Live", cls: "status-live", title: "USGS 3DEP terrain elevation", text: "Modelled terrain height and source acquisition date. It does not replace a survey.", links: [["Elevation point query", "https://epqs.nationalmap.gov/v1/docs"],["The National Map", "https://www.usgs.gov/programs/national-geospatial-program/national-map"]] },
  { id: "soil", status: "Live", cls: "status-live", title: "USDA Soil Data Access", text: "Mapped soil-unit components, drainage and hydrologic groups where recorded. It does not replace testing or a site investigation.", links: [["Soil Data Access", "https://sdmdataaccess.nrcs.usda.gov/"],["Web service documentation", "https://sdmdataaccess.nrcs.usda.gov/webservicehelp.aspx"]] },
...
export default function ResourcesPage() {
  return (
    <main className="product-page resources-page">
      ...
      <section className="product-section soft" id="disclaimer"><div className="portal-container"><p className="plain-callout"><strong>Important:</strong> UtilityDataUSA does not locate underground lines and does not replace 811, field markings, private utility locating, potholing, engineering review, permits or any clearance required by law or project conditions.</p><p className="fine-print" id="privacy">Searches send the submitted address to the U.S. Census geocoder and the matched coordinates to the relevant public-data providers. Successful matched-address reports and their public-source results may be saved for 30 days and reused according to each source’s freshness limit. The database stores the matched public address, source evidence and check times; it does not store the original typed text or associate reports with a user account. Saved report links allow anyone holding the link to reopen that evidence until expiry. Hosting and data providers may retain operational logs under their own policies. If you select AI interpretation, the address and source evidence are sent to OpenAI with response storage disabled; provider operational retention can still apply. Downloaded reports contain the matched address. A saved report link grants access to that report; an ordinary search link contains the address. Avoid entering confidential information. Contact info@brixcare.dk about privacy.</p><p className="fine-print" id="accessibility">Accessibility is part of the product design. If you encounter a barrier, contact info@brixcare.dk.</p><p className="fine-print" id="help">Questions or source suggestions: info@brixcare.dk.</p></div></section>
      <SiteFooter />
    </main>
  );
}
